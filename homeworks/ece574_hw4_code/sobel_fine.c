...
struct convolve_data_t {
	...
	struct image_t *new;
	int (*filter)[3][3];
	int thread_number;	// Added the thread number
	int ysize;		// And the size that the thread must go for
				// aka how many times y runs
};
...
static void *generic_convolve(void *argument) {
	...
	int thread_num = (data->thread_number) + 1; 	// Number of threads + 1 (used for calculations)
	int ystart;	// What y pixel to start on
	int ystop;	// What y pixel to stop on 

	// If a non power of 2 integer is used for the threads
	// make the last section (the ith section) have
	// a little extra to calculate the convolution for
	if((NUM_THREADS % 2) != 0){
		if((thread_num + 1) == NUM_THREADS) {
			ystop = ((data->ysize) * thread_num);
		      	ystop = ystop + ((old->y) - ystop);
		}
		else {
			ystop = (data->ysize) * thread_num - 1; 
		}
	}
	...
	// start at 1 pixel in for y
	if(data->thread_number == 0) {
		ystart = 1;
	}
	// Else start at where the last thread left off
	else {
		ystart = data->ysize * data->thread_number;
	}

	// Do the convolution for sobel
	for(d=0;d<3;d++) {
	   for(x=1;x<old->x-1;x++) {
	...
	// Print the thread number that finished
...
int main(int argc, char **argv) {
	...
	struct convolve_data_t sobel_data[NUM_THREADS];
	...
	// USED FOR PAPI
	long long time_diff, start, stop;
	...
	// Find how long the load image took
	...
	/* convolution */
	// Create NUM_THREADS amount threads for sobel x 
	// Get the real time in us
	start = PAPI_get_real_usec();
	for(t = 0; t < NUM_THREADS; t++) {

		sobel_data[t].old=&image;
		sobel_data[t].new=&sobel_x;
		sobel_data[t].filter=&sobel_x_filter;
		sobel_data[t].ysize = (image.y) / NUM_THREADS;	// Size to index is 1 / num_threads of ysize
		sobel_data[t].thread_number = t;		// Save the thread number
	
		printf("Creating Sobel x thread %d\n", t);
		my_value = pthread_create(
				&threads[t],
				NULL,
				generic_convolve,
				(void *)&sobel_data[t]);
	...
	// Join the threads together (if more than 1)
	if(NUM_THREADS > 1) {
		for(t = 0; t < NUM_THREADS; t++) {
			pthread_join(threads[t],NULL);
		}
	}
	// Else only join the first thread
	else {
		pthread_join(threads[0],NULL);
	}

	// Create NUM_THREADS amount of threads for sobel y
	for(t = 0; t < NUM_THREADS; t++) {

		sobel_data[t].old=&image;
		sobel_data[t].new=&sobel_y;
		sobel_data[t].filter=&sobel_y_filter;
		sobel_data[t].ysize = (image.y) / NUM_THREADS;	// Size to index is 1 / num_threads of ysize
		sobel_data[t].thread_number = t;		// Save the thread number
	
		printf("Creating Sobel y thread %d\n", t);
		my_value = pthread_create(
				&threads[t],
				NULL,
				generic_convolve,
	...
	// Get the real time again and show how long 
	// the convolve took
	stop = PAPI_get_real_usec();
	time_diff = stop - start;
	printf("Total convolution time: %lld us\n", time_diff);
	
	// Repeat the for combine and store
	start = PAPI_get_real_usec();
